...
class QualysVulnAdmin(admin.ModelAdmin):
	...
	search_fields = ('raw',)
	list_filter = ('severity', 'status', 'known_exploited', 'public_facing', 'os', 'first_seen', 'last_seen')
...
@admin.register(QualysVulnRiskAcceptance)
...
class AzureNamedLocationsAdmin(admin.ModelAdmin):
	...
	search_fields = ('displayName', 'ipNamedLocation_id',)
...
class SystemAdmin(SimpleHistoryAdmin):
	...
	fieldsets = (
		...
		('Brukerperspektivet og tilgangsstyring', {
			'fields': (
				('tilgangsgrupper_ad', 'antall_brukere'),
				'kontaktgruppe_url',
