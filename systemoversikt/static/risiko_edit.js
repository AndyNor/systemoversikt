...
// 2026-08-13: Compact unntak rows with button actions; gyldig_til on create form.
// 2026-08-13: Unntak CRUD on action cards; kontinuerlig_oppfolging status badge.
  ...
  function initRisikoEditor() {
    ...
    function collectScenarioPayload() {
      ...
      return {
        ...
        kit_dimensjoner: getKitDimensjonerFromToggles(),
    ...
    const KIT_ORDER = ['K', 'I', 'T'];

    function parseKitDimensjoner(text) {
      const seen = {};
      const result = [];
    ...
    function getKitDimensjonerFromToggles() {
      const selected = [];
      KIT_ORDER.forEach(function (code) {
        const btn = document.querySelector('#risiko-kit-toggles [data-kit="' + code + '"]');
    ...
    function setKitTogglesFromString(text) {
      const active = {};
      parseKitDimensjoner(text).forEach(function (code) {
        active[code] = true;
      });
      KIT_ORDER.forEach(function (code) {
        const btn = document.querySelector('#risiko-kit-toggles [data-kit="' + code + '"]');
    ...
    function buildModalActionCardHtml(action, defaultScenarioId) {
      ...
      const storedAnsvarlig = action.ansvarlig || '';
      const visibleAnsvarlig = action.ansvarlig_display || storedAnsvarlig;
      ...
      return '<div class="risiko-action-desc-row">' +
        ...
        '<div class="form-row risiko-action-meta-row no-gutters align-items-end">' +
          '<div class="col risiko-action-meta-col">' +
            '<div class="risiko-action-ansvarlig-wrap">' +
            '<label class="risiko-action-label">Ansvarlig</label>' +
            '<input type="search" class="form-control form-control-sm risiko-action-ansvarlig-input" ' +
            'value="' + escapeHtml(visibleAnsvarlig) + '" ' +
            'data-ansvarlig-value="' + escapeHtml(storedAnsvarlig) + '"' + ansvarligLabelAttr + ' ' +
            'placeholder="Søk navn eller e-post">' +
            '<div class="risiko-action-ansvarlig-treff"></div>' +
            '</div>' +
          '</div>' +
          '<div class="col risiko-action-meta-col">' +
    ...
    function buildActionUnntakHtml(action) {
      ...
      const items = action.unntak || [];
      const count = action.unntak_count != null ? action.unntak_count : items.length;
      let listHtml = '';
      if (items.length) {
        listHtml = '<ul class="list-unstyled mb-1 risiko-action-unntak-list">' +
          items.map(function (u) {
            const aktivBadge = u.aktiv
              ? '<span class="badge badge-success">Aktiv</span>'
              : '<span class="badge badge-secondary">Lukket</span>';
            const gyldig = u.gyldig_til
              ? '<span class="text-muted risiko-unntak-gyldig" title="Gyldig til">til ' +
                escapeHtml(u.gyldig_til) + '</span>'
              : '';
            const begrunnelse = u.begrunnelse
              ? '<span class="text-muted risiko-unntak-begrunnelse-inline" title="' +
                escapeHtml(u.begrunnelse) + '"> – ' + escapeHtml(u.begrunnelse) + '</span>'
              : '';
            return '<li class="risiko-unntak-row" data-unntak-id="' + u.id + '"' +
              ' data-beskrivelse="' + escapeHtml(u.beskrivelse || '') + '"' +
              ...
              '<strong>' + escapeHtml(u.beskrivelse || '') + '</strong>' +
              begrunnelse +
              '</span>' +
              gyldig +
              '</div>' +
        ...
        listHtml = '<p class="text-muted small mb-1">Ingen unntak registrert.</p>';
      }
      return '<div class="risiko-action-unntak mt-1" data-action-id="' + action.id + '">' +
        '<div class="d-flex align-items-center mb-1">' +
        '<strong class="small mb-0">Unntak' +
        (count ? ' <span class="badge badge-light border">' + count + '</span>' : '') +
        '</strong>' +
        '<button type="button" class="btn btn-outline-secondary btn-sm ml-auto risiko-unntak-toggle-form">Legg til</button>' +
        '</div>' +
        listHtml +
    ...
    function unntakListCreateUrl(actionId) {
      return (config.urls.unntakListCreate || '').replace('{id}', String(actionId));
    }

    function unntakDetailUrl(actionId, unntakId) {
      return (config.urls.unntakDetail || '')
        .replace('{id}', String(actionId))
        .replace('{uid}', String(unntakId));
    }

    function refreshActionUnntakBlock(card, actionId) {
      if (!config.urls.unntakListCreate) return Promise.resolve();
      return fetchJson(unntakListCreateUrl(actionId)).then(function (data) {
        const block = card.querySelector('.risiko-action-unntak');
        if (!block) return;
        const action = {
          id: actionId,
          unntak: data.unntak || [],
          unntak_count: (data.unntak || []).length,
        };
        const wrapper = document.createElement('div');
        wrapper.innerHTML = buildActionUnntakHtml(action);
    ...
    function collectAnsvarligFromCard(card) {
      const input = card.querySelector('.risiko-action-ansvarlig-input');
      if (!input) return '';
      return (input.getAttribute('data-ansvarlig-value') || input.value || '').trim();
    }

    function syncAnsvarligStoredValue(input) {
      const trimmed = input.value.trim();
      const pickedLabel = input.getAttribute('data-ansvarlig-label');
      if (pickedLabel && trimmed === pickedLabel) {
        return;
      }
      input.setAttribute('data-ansvarlig-value', trimmed);
      input.removeAttribute('data-ansvarlig-label');
    }

    const ansvarligSearchTimers = new WeakMap();

    function scheduleAnsvarligSearch(input) {
      const wrap = input.closest('.risiko-action-ansvarlig-wrap');
      if (!wrap || !config.urls.tiltakAnsvarligSearch) return;
      const treff = wrap.querySelector('.risiko-action-ansvarlig-treff');
      const q = input.value.trim();
      const existing = ansvarligSearchTimers.get(input);
      if (existing) clearTimeout(existing);
      if (q.length < 2) {
        if (treff) treff.innerHTML = '';
        return;
      }
      ansvarligSearchTimers.set(input, setTimeout(function () {
        ansvarligSearchTimers.delete(input);
        fetchJson(config.urls.tiltakAnsvarligSearch + '?q=' + encodeURIComponent(q))
          .then(function (data) {
            if (!treff) return;
            treff.innerHTML = '';
            (data.results || []).forEach(function (hit) {
              ...
              btn.type = 'button';
              btn.className = 'btn btn-link btn-sm d-block text-left p-0 risiko-tiltak-ansvarlig-pick';
              btn.setAttribute('data-email', hit.email || '');
              btn.setAttribute('data-label', hit.label || hit.email || '');
              btn.textContent = hit.label || hit.email || '';
    ...
    function buildTiltakReadOnlyRowHtml(action) {
      ...
      const ansvarligRaw = action.ansvarlig || '';
      const ansvarligShow = action.ansvarlig_display || ansvarligRaw || '-';
      let ansvarligHtml = escapeHtml(ansvarligShow);
      if (ansvarligRaw && ansvarligShow !== ansvarligRaw) {
        ansvarligHtml = '<span title="' + escapeHtml(ansvarligRaw) + '">' + escapeHtml(ansvarligShow) + '</span>';
      }
      return '<td>' + displayId + '</td>' +
        '<td>' + escapeHtml(action.beskrivelse || '').replace(/\n/g, '<br>') +
        (action.unntak_count ? ' <span class="badge badge-light border" title="Aktive/registrerte unntak">' +
          action.unntak_count + ' unntak</span>' : '') +
        '</td>' +
        '<td>' + riskHtml + '</td>' +
        '<td>' + ansvarligHtml + '</td>' +
    ...
    const kitToggles = document.getElementById('risiko-kit-toggles');
    if (kitToggles) {
      kitToggles.addEventListener('click', function (e) {
        const btn = e.target.closest('.risiko-kit-toggle');
        if (!btn) return;
    ...
    if (actionsList) {
      actionsList.addEventListener('input', function (e) {
        ...
        if (e.target.classList.contains('risiko-action-ansvarlig-input')) {
          syncAnsvarligStoredValue(e.target);
          scheduleAnsvarligSearch(e.target);
        }
      ...
      actionsList.addEventListener('click', function (e) {
        const card = e.target.closest('.risiko-action-card');
        if (e.target.classList.contains('risiko-tiltak-ansvarlig-pick')) {
          const wrap = e.target.closest('.risiko-action-ansvarlig-wrap');
          if (wrap && card) {
            const input = wrap.querySelector('.risiko-action-ansvarlig-input');
            const label = e.target.getAttribute('data-label') || '';
            const email = e.target.getAttribute('data-email') || '';
            if (input) {
              input.setAttribute('data-ansvarlig-value', email);
              input.setAttribute('data-ansvarlig-label', label);
              input.value = label;
            }
            const treff = wrap.querySelector('.risiko-action-ansvarlig-treff');
            if (treff) treff.innerHTML = '';
        ...
        if (e.target.classList.contains('risiko-unntak-save')) {
          ...
          fetchJson(unntakListCreateUrl(actionId), {
            ...
            setModalStatus(err.message || 'Kunne ikke lagre unntak.', true);
          });
          return;
        }
        if (e.target.classList.contains('risiko-unntak-delete') || e.target.classList.contains('risiko-unntak-toggle-aktiv')) {
          const actionId = parseInt(card.getAttribute('data-action-id'), 10);
          const item = e.target.closest('[data-unntak-id]');
          if (!actionId || !item || !config.urls.unntakDetail) return;
          const unntakId = parseInt(item.getAttribute('data-unntak-id'), 10);
          if (e.target.classList.contains('risiko-unntak-delete')) {
            if (!window.confirm('Slette dette unntaket?')) return;
            fetchJson(unntakDetailUrl(actionId, unntakId), {
              method: 'POST',
              body: JSON.stringify({ _method: 'DELETE' }),
            }).then(function (data) {
              ...
              setModalStatus(err.message || 'Kunne ikke slette unntak.', true);
            });
            return;
          }
          const currentlyAktiv = item.getAttribute('data-aktiv') === '1';
          fetchJson(unntakDetailUrl(actionId, unntakId), {
            method: 'PATCH',
            body: JSON.stringify({
              beskrivelse: item.getAttribute('data-beskrivelse') || '',
              begrunnelse: item.getAttribute('data-begrunnelse') || '',
              gyldig_til: item.getAttribute('data-gyldig-til') || null,
              aktiv: !currentlyAktiv,
            }),
          }).then(function (data) {
            if (data.tiltak) {
