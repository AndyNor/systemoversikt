	...
	class CustomOIDCAuthenticationBackend(OIDCAuthenticationBackend):
		def filter_users_by_claims(self, claims):
			...
			try:
				...
				message = "%s kunne ikke logge inn. Kobling mot brukerID feilet" % username
				ApplicationLog.objects.create(event_type="Brukerpålogging", message=message)
				messages.warning(self.request, 'Fant ingen korresponderende bruker. Innlogging feilet.')
				return self.UserModel.objects.none()
